...
sealed class ControlStyle(
    ...
    data class Quote(val selectedText: String?) : ControlStyle(
        style = "<div style=\"background-color:#FAFAFA" +
                ";padding:12px;border-radius:6px;\"><em>❞" +
                " $selectedText</em></div>"
    )

    data class Code(val selectedText: String?) : ControlStyle(
        style = "<div style=\"background-color:#0d1117" +
                ";padding:12px;border-radius:6px;\">" +
                "<pre><code class=\"language-kotlin\"> $selectedText </code></pre></div>"
    )
    ...
    data class Image(
        ...
        style = "<img src=\"$imageUrl\" alt=\"$alt\" " +
                "style=\"max-width: 100%\">$selectedText</img>"
    )
